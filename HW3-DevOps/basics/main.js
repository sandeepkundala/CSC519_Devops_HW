...
app.use(function (req, res, next) {
  ...
  if (req.url != "/favicon.ico"){
    client.lpush(['string SITES', req.url.toString()], function(err, reply){
      if (reply){
        client.ltrim('string SITES',0,4, function(err, reply){
          if (reply){
...
app.get('/set', function (req, res) {
  res.writeHead(200, { 'content-type': 'text/html' });
  client.set('string key', 'this message will self-destruct in 10 seconds', function (err, reply) {  });
...
app.get('/recent', function (req, res) {
  let siteList = "";
  res.writeHead(200, { 'content-type': 'text/html' });
  client.LRANGE("string SITES",1, -1, function(err, reply){
    reply.forEach(function(r) {
      siteList += r.toString() + " ";
    })
    siteList = siteList.trim();
